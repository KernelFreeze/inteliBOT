// DotNetWikiBot Framework 3.10 - designed to make robots for MediaWiki-powered wiki sites
...
namespace DotNetWikiBot
{
	...
	public class Site
	{
		...
		private void Initialize()
		{
			...
			if (File.Exists(cacheFile) == true) {
				...
				if (address.Contains("sourceforge.net")) {
					indexPath = address.Substring(0, address.LastIndexOf('/') + 1) + "index.php";
				}
				else {
					string src = GetWebPage(address);
					Regex indexPathRegex = new Regex("(?i) href=\"(((" + Regex.Escape(address) +
						"|/|\\.)[^\"]*)?(index|api)\\.php)");
					try {
						foreach (Match m in indexPathRegex.Matches(src)) {
							if (m.Groups[1].Value.StartsWith("http"))
								indexPath = m.Groups[1].Value;
							else
								indexPath = new Uri(new Uri(address), m.Groups[1].Value).ToString();
						}
					}
					catch {
		...
		public Dictionary<string,string> LoadMediawikiMessages(bool modified)
		{
			...
			Console.WriteLine(Bot.Msg("MediaWiki system messages were loaded successfully."));
		...
		{
			if (address.Contains("sourceforge.net")) {
				string src = PostDataAndGetResult("https://sourceforge.net/account/login.php",
					"", true, true);
				Match match = Regex.Match(src, "name=\"_visit_cookie\" value=\"([^\"]+)");
				string postData = string.Format("form_loginname={0}&form_pw={1}&_visit_cookie={2}" +
					"&ssl_status=&remember_usernamepwd=yes&login=Log+in",
					HttpUtility.UrlEncode(userName.ToLower()),
					HttpUtility.UrlEncode(userPass),
					match.Groups[1].Value);
				string respStr = PostDataAndGetResult("https://sourceforge.net/account/login.php",
					postData, true, true);
	...
	public class Page
	{
		...
		/// It is slower than Load(), it should be used only if page's metadata is required
		...
		/// <param name="comment">Your edit comment.</param>
		/// <param name="isMinorEdit">Minor edit mark (true = minor edit).</param>
		/// <param name="reviewVersion">If true, the page version after this edit is marked
		/// reviewed. Bot's account must have sufficient rights to mark versions reviewed.
		...
		public void Watch()
		{
			...
			string watchToken = "";
			Regex watchTokenRegex =
				new Regex("(?:&|&amp;|\\?)action=watch&(?:amp;)?token=([^\"]+?)\"");
			if (watchTokenRegex.IsMatch(respStr))
				watchToken = watchTokenRegex.Match(respStr).Groups[1].ToString();
					// token is url-encoded
			respStr = site.GetWebPage(res + "&action=watch&token=" + watchToken);
			watched = true;
			if (site.watchList == null)
				site.watchList.FillFromWatchList();
		...
		public void Unwatch()
		{
			...
			string respStr = site.GetWebPage(res);
			string unwatchToken = "";
			Regex unwatchTokenRegex =
				new Regex("(?:&|&amp;|\\?)action=unwatch&(?:amp;)?token=([^\"]+?)\"");
			if (unwatchTokenRegex.IsMatch(respStr))
				unwatchToken = unwatchTokenRegex.Match(respStr).Groups[1].ToString();
					// token is url-encoded
			respStr = site.GetWebPage(res + "&action=unwatch&token=" + unwatchToken);
			watched = false;
		...
		public void FillFromCustomApiQuery(string query, string queryParams, int limit)
		{
			...
			if (!string.IsNullOrEmpty(Environment.StackTrace)
				...
				&& !Environment.StackTrace.Contains("FillAllFrom"))
					Console.WriteLine(string.Format(Bot.Msg("PageList filled with {0} page " +
		...
		public void FillFromPagesUsingImage(string imageFileTitle)
		{
			...
			if (!site.useApi) {
				FillFromCustomApiQuery("list=imageusage", "iutitle=" +
					HttpUtility.UrlEncode(site.GetNsPrefix(6)) + imageFileTitle, int.MaxValue);
			}
			else {    // TO DO: paging
				string res = site.indexPath + "?title=" +
				...
				try {
					...
					Console.Error.WriteLine(
						Bot.Msg("No page contains \"{0}\" image."), imageFileTitle);
					return;
				}
	...
	public class Bot
	{
		...
		public static readonly string botVer = "inteliBOT";
		/// <summary>Version of DotNetWikiBot Framework.</summary>
		public static readonly Version version = new Version("1.0.2.1");
		/// <summary>Local cache directory.</summary>
